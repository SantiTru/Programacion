...
//Switch que se encarga de ejecutar las funciones segun la opcion que se le pase por parametro.
            switch (opcion[i]) {
...
*/
    private static String obtenerNombreFichero(String[] args) {
        return "Tema11intro.txt";
    }
/* 
@info: funcion que cuenta la palabra introducida por linea de comandos en el texto.  
@param args: ArrayList de lineas del texto, palabraBuscada (introducida por linea de comandos)
@name: contarPalabras
*/
...
@param args: ArrayList de lineas del texto, palabraBuscada (introducida por linea de comandos), constante ROJO
@name: colorearPalabras
@return: contador de palabras coloreadas por la palabra introducida por linea de comandos
*/
    private static int colorearPalabras (ArrayList<String> texto, String palabraBuscada, String ROJO) {
        int colorea = 0; //variable que contiene el contador de colorea
            for (int i = 0; i < texto.size(); i++) { //Bucle for que recorre el ArrayList de lineas del texto
                if (texto.get(i).toLowerCase().contains(palabraBuscada.toLowerCase())) { //Para no distinguir mayúsculas-minúsculas
                    ...
                    colorea++; //aumenta el contador de colorea para saber cuantas veces se ha coloreada la palabra.
                }
            }
...
/* 
@info: funcion que devuelve la posición de la palabra introducida por linea de comandos en el texto.  
@param args: ArrayList de lineas del texto, palabra (introducida por linea de comandos)
@name: DevolverPosicion
*/
    private static void DevolverPosicion(ArrayList<String> texto, String Palabra) {
//Variables para almacenar la posición de la palabra introducida por linea de comandos en el texto
        int cont=0;
        int posicion=0;
        System.out.println("--------------------------------------------");
        System.out.println("Posiciones: ");
        for (String i: texto) { //Bucle for que recorre el ArrayList de lineas del texto
//Tenemos dos if anidados que contabilizan primera la posición y después comprueba si la palabra se encuentra en dicha posición
            if (i.toLowerCase().contains(SEPARADOR)) {
                cont++;
            
            }if (i.toLowerCase().contains(Palabra.toLowerCase())) {
                posicion=cont;          
//Imprime la posición de la palabra introducida por linea de comandos en el texto si se ha encontrado coincidencia.
...
/* 
@info: funcion que devuelve la linea donde se encuentra la palabra introducida por linea de comandos en el texto.  
@param args: ArrayList de lineas del texto, palabra (introducida por linea de comandos)
@name: DevolverLinea
*/
    private static void DevolverLinea(ArrayList<String> texto, String Palabra) { 
//Variable para almacenar la linea donde se encuentra la palabra introducida por linea de comandos en el texto
        int linea=1;
        System.out.println("--------------------------------------------");
        System.out.println("Lineas que contienen la palabra: ");
        for (String i: texto) { //Bucle for que recorre el ArrayList de lineas del texto
//Similar a la función anterior tenemos dos if anidados que contabilizan primera la linea y aumenta el contador y después comprueba si la palabra se encuentra en dicha linea
            if (i.contains("\n")) {
                linea++;
            }if (i.toLowerCase().contains(Palabra.toLowerCase())) {           
//Imprime la linea de la palabra introducida por linea de comandos en el texto si se ha encontrado coincidencia.                
...
//Mediante un bucle for que recorre el ArrayList de lineas del texto almacenamos en un String temporal el contenido de cada linea.
        for (int i = texto.size() - 1; i > 0; i--) {
            int indice = desorden.nextInt(i + 1);
            String temporal = texto.get(i);
//Despúes lo cambiamos con la función set de ArrayList para que se desordenen las lineas del texto
...
//Al igual que en la función anterior tenemos un bucle for que recorre el ArrayList de lineas del texto y almacena en un String temporal el contenido de cada linea pasado a minúsculas.
        for (int i = 0; i < texto.size(); i++) {
            textoMinusculas=texto.get(i).toLowerCase();
//En cada iteración imprimimos el String temporal en minésculas para que se vaya imprimiendo el texto completo.
...
//Al igual que en la función anterior tenemos un bucle for que recorre el ArrayList de lineas del texto y almacena en un String temporal el contenido de cada linea pasado a mayúsculas.
        for (int i = 0; i < texto.size(); i++) {
...
*/
    public static void imprimir(ArrayList<String> texto) {
        for (int i = 0; i < texto.size(); i++) { //Bucle for que recorre el ArrayList de lineas del texto
//En cada iteración imprimimos el String por el que ha pasado el bucle para que se vaya imprimiendo el texto completo.
