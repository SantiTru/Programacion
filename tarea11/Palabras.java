...
public class Palabras {
    ...
    public static void main(String[] args) {
        ...
        String nombreFichero = obtenerNombreFichero(args); // Argumento -f 
        String opcion[] = args;
        ArrayList<String> texto = null;//Definimos el ArrayList como null para poder introducirlo en las funciones.
        // Lo metemos en un try-catch para que si hay un error, nos avise.
        try{
            texto = leerTodoFichero(nombreFichero); // DEFINITIVO
        // En el catch capturamos la excepción que puede ocurrir y lo mostramos por pantalla.
        } catch (IOException e) {
            System.out.println("Mensaje de error " + e.getMessage());
            return; // aqui detenemos el programa ya que hay un error. 
        } 
     ...
     */
    public static ArrayList<String> leerTodoFichero(String nombreFichero) throws IOException {
    ...
    // bucle for buscando -f y en la siguiente posición (args [i+1] es el archivo)                                           
            if (args[i].equals("-f")) {
                if (i == args.length - 1) {
            //Si encuentra -f pero no se encuentra el archivo nos devuelve este mensaje
                    System.out.println("No se ha introducido un archivo despues del -f.");
                } else {
            //Si encuentra -f nos devuelve el nombre del archivo que es el siguiente argumento dado
        ...
        if (nombreArchivo.equals("")) {
            //Si no encuentra -f nos devuelve este mensaje
            System.out.println("Es necesario establecer correctamente los comandos: (Ejemplo) -f archivo.txt");
        }
    ...
    public static void contarPalabras(ArrayList<String> texto, String palabraBuscada) {
        ...
        for (String palabra : texto) { // Bucle for que recorre el ArrayList de lineas del texto
            if (palabra.toLowerCase().contains(palabraBuscada.toLowerCase())) { // Para no distinguir
    ...
    public static void Contador(ArrayList<String> texto) {
        ...
        for (String i : texto) { // Bucle for que recorre el ArrayList de lineas del texto
            if (i.toLowerCase().contains(SEPARADOR)) { // Para no distinguir mayúsculas-minúsculas pasamos por
                                                       // toLowerCase
